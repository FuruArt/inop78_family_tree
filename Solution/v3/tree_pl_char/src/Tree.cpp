...
void Tree::memberTree(Pointer member)
{
	...
	for (auto it = _tree.begin(); it != _tree.end(); it++) {
		int vector = 0;
		for (Person person : _tree[it->first]) {
			if ((person.getFather().map == father.map && person.getFather().vector == father.vector && person.getFather().map != 404)
				|| (person.getMother().map == mother.map && person.getMother().vector == mother.vector && person.getMother().map != 404)) {
